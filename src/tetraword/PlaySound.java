...
import javax.sound.sampled.UnsupportedAudioFileException;
 ...
 public class PlaySound {    
     ...
     private ThreadPlaySound t = new ThreadPlaySound();
     ...
     public static void main(String [] args) throws Exception{
	    	int i =0; 
	          PlaySound wp = new PlaySound(new File("src/audio/right.wav"));
	          wp.open();
	          wp.play();
	          PlaySound w1p = new PlaySound(new File("src/audio/right.wav"));
	          w1p.open();
	          w1p.play();
    	 while(i==0){

	          if(ThreadPlaySound.endMusic){
	        	  wp = new PlaySound(new File("src/audio/pirate.wav"));
	        	  wp.open();
	        	  wp.play();
